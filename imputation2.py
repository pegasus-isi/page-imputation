...
def getDAX( genotype_file,
    ...
    for chromosome in range( chromosome_start, chromosome_end + 1 ):
        preprocessing_job = construct_preprocessing_job( prefix, chromosome )
        dax.addJob( preprocessing_job )

        extract_job = construct_extract_job( prefix, chromosome )
        dax.addJob( extract_job )
        dax.depends( parent=preprocessing_job, child=extract_job)
...
def construct_preprocessing_job( prefix, chromosome_num ):
    """
    ...
    # the input files
    for suffix in [ ".fam", ".bim", ".bed"]:
        input = prefix + suffix
        j.uses( File( input ), link=Link.INPUT)

    genotype_vcf_gz = "ALL." + chromosome_name + ".phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf.gz"

    # add other fixed input file lfns
    for input in [ genotype_vcf_gz, "hg38ToHg19.over.chain.gz",
    ...
    #the output file
    genotype_lfn = get_variant_call_format_lfn( prefix, chromosome_num )
...
def construct_imputation_job( prefix, chromosome_num, reference_file_prefix, snps_exclude_lfn, chunk_start, chunk_end ):
    ...
    for suffix in [".impute2.gz", ".impute2_diplotype_ordering" ,".impute2_info" ,".impute2_info_by_sample", ".impute2_summary", ".impute2_warnings"]:
...
def get_variant_call_format_lfn( prefix, chromosome_num):
    """
    Returns the name of the vcf file generated by the preprocessing step
    """
    return prefix + "." + str(chromosome_num)  + ".vcf.gz"
