...
plink --memory ${PLINK_MEMORY} --bfile $file --update-map ${study}.dbSNP_rs_lifted.out --update-name --make-bed --out ${study}.TEMP1
...
plink --memory  ${PLINK_MEMORY} --bfile ${study}.TEMP2 --exclude ${study}.temp1.out --make-bed --out ${study}.TEMP3
## 3b. To remove those with 3 dupplicates, remain one of them
sort ${study}.TEMP3.bim | uniq -d | awk '{print $2}' > ${study}.temp2.out
plink --memory  ${PLINK_MEMORY} --bfile ${study}.TEMP3 --exclude ${study}.temp2.out --make-bed --out ${study}.TEMP4
