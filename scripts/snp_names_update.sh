...
plink --memory --bfile $file --update-map ${study}.dbSNP_rs_lifted.out --update-name --make-bed --out ${study}.TEMP1
...
plink --memory --bfile ${study}.TEMP1 --update-map ${study}.temp2.out --update-name --make-bed --out ${study}.TEMP2
...
plink --memory --bfile ${study}.TEMP2 --exclude ${study}.temp1.out --make-bed --out ${study}.TEMP3
## 3b. To remove those with 3 dupplicates, remain one of them
sort ${study}.TEMP3.bim | uniq -d | awk '{print $2}' > ${study}.temp2.out
plink --memory --bfile ${study}.TEMP3 --exclude ${study}.temp2.out --make-bed --out ${study}.TEMP4
...
plink --memory --bfile ${study}.TEMP4 --exclude ${study}.temp3.out --make-bed --out $study
